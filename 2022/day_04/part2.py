...
def overlaps(first: range, second: range):

    if first.start <= second.start and first.stop >= second.start:
        return True

    if first.start <= second.stop and first.stop >= second.stop:
        return True

    if second.start <= first.start and second.stop >= first.start:
        return True

    if second.start <= first.stop and second.stop >= first.stop:
        return True
